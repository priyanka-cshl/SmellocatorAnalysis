function [handles] = AlignVideoAndTraces240319(write_video)
...
if write_video
    ...
    writerObj = VideoWriter('N8_withPassiveReplay_slow3.avi');
...
%% figures with static images
nplots = 3; counter = 0;
% figure('position', [1 1 framedimensions(2) framedimensions(1)], 'color', 'w');
% colormap('gray');
FrameList = sortrows(FrameList,3);
temp = FrameList;
% for f = 1:size(FrameList,1)
%     
%     if rem(f,nplots) == 1
%         figure('position', [1 1 framedimensions(2) framedimensions(1)], 'color', 'w');
%         colormap('gray');
...
%     frame = imread(fullfile(VideoFolder{1},[filetag{1},sprintf('%04d',frameID),'.png']));
%     frame = imresize(frame,0.5);
%     subplot(2,nplots,nplots+counter)
%     imagesc(frame);
%     set(gca,'XTick',[],'XTickLabel',' ','XTickMode','manual','XTickLabelMode','manual');
%     set(gca,'YTick',[],'YTickLabel',' ','YTickMode','manual','YTickLabelMode','manual');
%     
...
%%
% favFrames = [3 5 17 21 23 26 29 38 47];
% FrameList = FrameList(favFrames,:);
% figure('position', [1 1 framedimensions(2) framedimensions(1)], 'color', 'w');
% colormap('gray');
% counter = 0;
% for f = 1:size(FrameList,1)
...
%     set(gca,'XTick',[],'XTickLabel',' ','XTickMode','manual','XTickLabelMode','manual');
%     set(gca,'YTick',[],'YTickLabel',' ','YTickMode','manual','YTickLabelMode','manual');
%     set(gca,'Position',[(0.01 + (counter-1)*0.1), 0.5, 0.09, 0.9*screenratio*ratio/10]);
%     
%     subplot(2,9,counter+9);
%     frameID = FrameList(f,2);
%     frame = imread(fullfile(VideoFolder{1},[filetag{1},sprintf('%04d',frameID),'.png']));
%     frame = imresize(frame,0.5);
%     imagesc(frame);
%     set(gca,'XTick',[],'XTickLabel',' ','XTickMode','manual','XTickLabelMode','manual');
%     set(gca,'YTick',[],'YTickLabel',' ','YTickMode','manual','YTickLabelMode','manual');
%     set(gca,'Position',[(0.01 + (counter-1)*0.1), 0.3, 0.09, 0.9*screenratio*ratio/10]);
% end
...
colormap('gray');
counter = 0;
picwidth = 1/numel(favFrames+1);
nPlots = numel(favFrames);
...
for f = 1:size(FrameList,1)
    ...
    set(gca,'XTick',[],'XTickLabel',' ','XTickMode','manual','XTickLabelMode','manual');
    set(gca,'YTick',[],'YTickLabel',' ','YTickMode','manual','YTickLabelMode','manual');
    set(gca,'Position',[(0.005 + (counter-1)*picwidth), 0.51, picwidth-0.015, picwidth*screenratio*ratio]);
    
    subplot(2,nPlots,counter+nPlots);
    frameID = FrameList(f,2);
    frame = imread(fullfile(VideoFolder{1},[filetag{1},sprintf('%04d',frameID),'.png']));
    frame = imresize(frame,0.5);
    imagesc(frame);
    set(gca,'XTick',[],'XTickLabel',' ','XTickMode','manual','XTickLabelMode','manual');
    set(gca,'YTick',[],'YTickLabel',' ','YTickMode','manual','YTickLabelMode','manual');
    set(gca,'Position',[(0.005 + (counter-1)*picwidth), 0.01, picwidth-0.015, picwidth*screenratio*ratio]);
...
%% axes to plot the odor flow 
% handles.Odor = axes;
% handles.Odor.Position(1) = handles.Camera1.Position(1);
% handles.Odor.Position(3) = handles.Camera1.Position(3);
% handles.Odor.Position(2) = handles.Camera1.Position(2) + 0.80*(handles.Camera1.Position(4));
% handles.Odor.Position(4) = 0.12;
% axes(handles.Odor);
% set(gca, 'Color', 'none');
% handles.Odor.YColor = 'none';
% handles.Odor.XColor = 'none';
% % handles.OdorFlow = imagesc((-50:1:50)/100,[-1 1]);
% % handles.OdorFlow.AlphaData = 0.5;
% % colormap(handles.Odor, brewermap(33, 'rdbu'));
% handles.OdorBox = rectangle('Position',[0 0 2 1],...
%     'FaceColor',[0.8 0.8 0.8 0.4], ...
%     'EdgeColor','k','Linewidth', 4);
% 
% set(handles.Odor,'XTick',[],'XTickLabel',' ','XTickMode','manual','XTickLabelMode','manual');
% set(handles.Odor,'YTick',[],'YTickLabel',' ','YTickMode','manual','YTickLabelMode','manual','TickDir','out','Box','off');
