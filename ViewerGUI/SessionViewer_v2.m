function varargout = SessionViewer_v2(varargin)
% SESSIONVIEWER_V2 MATLAB code for SessionViewer_v2.fig
%      SESSIONVIEWER_V2, by itself, creates a new SESSIONVIEWER_V2 or raises the existing
%      singleton*.
%
%      H = SESSIONVIEWER_V2 returns the handle to a new SESSIONVIEWER_V2 or the handle to
%      the existing singleton*.
%
%      SESSIONVIEWER_V2('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in SESSIONVIEWER_V2.M with the given input arguments.
%
%      SESSIONVIEWER_V2('Property','Value',...) creates a new SESSIONVIEWER_V2 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before SessionViewer_v2_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to SessionViewer_v2_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help SessionViewer_v2

% Last Modified by GUIDE v2.5 18-Mar-2024 11:36:35

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @SessionViewer_v2_OpeningFcn, ...
                   'gui_OutputFcn',  @SessionViewer_v2_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before SessionViewer_v2 is made visible.
function SessionViewer_v2_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to SessionViewer_v2 (see VARARGIN)

% Choose default command line output for SessionViewer_v2
...
% UIWAIT makes SessionViewer_v2 wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = SessionViewer_v2_OutputFcn(hObject, eventdata, handles) 
...
[Leverwarped,Odorwarped] = WarpTrialTraces(Traces, TrialInfo);
handles.TrialLims.Data = [1 size(Leverwarped,1)];
axes(handles.LeverPlot);
imagesc(Leverwarped');
colormap(gca,brewermap(20,'Blues'));

axes(handles.OdorPlot);
imagesc(Odorwarped');
colormap(handles.OdorPlot,brewermap(20,'RdBu'));
...
colormap(gca,brewermap(100,'RdYlBu'));
...
% --- Executes when entered data in editable cell(s) in TrialLims.
function TrialLims_CellEditCallback(hObject, eventdata, handles)
% hObject    handle to TrialLims (see GCBO)
% eventdata  structure with the following fields (see MATLAB.UI.CONTROL.TABLE)
%	Indices: row and column indices of the cell(s) edited
%	PreviousData: previous data for the cell(s) edited
%	EditData: string(s) entered by the user
...
%	Error: error string when failed to convert EditData to appropriate value for Data
% handles    structure with handles and user data (see GUIDATA)
set(handles.LeverPlot,'XLim',handles.TrialLims.Data);
set(handles.OdorPlot,'XLim',handles.TrialLims.Data);
