...
f = find(strcmp(ReplayInfo.Perturbation(:,1),'OL-Template'));
if ~isempty(f)
    for i = 1:numel(f)
        ReplayInfo.Perturbation{f(i),1} = [];
    end
end

thisUnitSpikes = ReplayAlignedSpikes(:,whichUnit);
whichodor = whichOdor;
% get the trials for that particular odor
allTrials = intersect(find(cellfun(@isempty, ReplayInfo.Perturbation(:,1))), ...
