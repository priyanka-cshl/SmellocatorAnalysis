...
function [CLsniffs, window] = PlotSniffRaster_wdw(SessionName, whichUnit, varargin)

%% parse input arguments
narginchk(1,inf)
params = inputParser;
params.CaseSensitive = false;
params.addParameter('plotClosedLoop', true, @(x) islogical(x) || x==0 || x==1);
params.addParameter('plotPassive', false, @(x) islogical(x) || x==0 || x==1);

% extract values from the inputParser
params.parse(varargin{:});
plotClosedLoop  = params.Results.plotClosedLoop;
plotPassive     = params.Results.plotPassive;
...
if plotClosedLoop
    ...
    for snifftype = 1:5
        ...
        Sniffidx(whichsniffs,7) = snifftype;
        Sniffidx(whichsniffs,8:9) = SniffTS(:,1:2);
