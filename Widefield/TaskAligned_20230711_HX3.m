...
%% get the processed behavior data loaded
...
%% possible behavior alignment points
% align to trial start - may be 100 ms before and after?
for TZ = 1:12
    whichtrials = intersect(find(TrialInfo.TargetZoneType==TZ), ...
        find(cellfun(@isempty, TrialInfo.Perturbation)));
    
    timestamps  = TrialInfo.SessionTimestamps(whichtrials,1:2); % Trial On and Off in session timestamps
    clear frame_ts
    for i = 1:size(timestamps,1)
        [~,frame_ts(i,1)] = min(abs(WF_timestamps.Behavior(:,1)-timestamps(i,1)));
        [~,frame_ts(i,2)] = min(abs(WF_timestamps.Behavior(:,1)-timestamps(i,2)));
        
    end
    
    % activity aligned to trial start
    clear AlignedMaps
    for i = 1:size(timestamps,1)
        baselineframes  = frame_ts(i,2) + [-5:1:-1];
        postframes      = frame_ts(i,2) + [1:5];
        baseline        = mean(WF_stack(:,:,baselineframes),3);
        postframe       = mean(WF_stack(:,:,postframes),3);
        deltaframe      = (postframe - baseline)./baseline;
        AlignedMaps(:,:,i) = baseline; 
    end
end
...
for roi = 1:length(selROIs)
    ...
    figure; 
    subplot(1,6,1);
    imagesc(reshape(Cmat(:,1),stackdims(1),stackdims(2)));
    colormap(brewermap([50],'*RdBu'));
    hold on
