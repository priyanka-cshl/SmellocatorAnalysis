function [MyTrials_m, TrialSequence_m, ReplayTraces, MyParams_m, TuningFileTS] = ParseTuningSession_Multi(FileName, PIDflag)

%% same as  ParseTuningSession.m but for cases where more than one passive replays were done
%%
...
ReplayTraces = [];
MyTrials_m = []; TrialSequence_m = [];  MyParams_m = []; TuningFileTS = [];

for n = 1:size(FileName,1)
    NameTag = [NameTag, char(regexp(FileName(n,:),'_o[0-9]+','match'))];
    
    MyTrials = []; TrialSequence = [];  MyParams = [];
    
    [MyData, MyParams, DataTags, TrialSequence, LocationSequence] = LoadSessionData(FileName(n,:), 1, PIDflag); %#ok<ASGLU>
    ...
    while TrialOn(end)>TrialOff(end)
        %TrialOn(end,:) = []; 
        %PG: We want to keep these trials when there
        %are multiple tuning files such that all TTLs on ephys side are
        %accounted for
        TrialOff(end+1) = length(TrialColumn);
    end
    ...
    if any(TrialSequence(:,1)==999)
        ...
        if ~isempty(x)
            ...
            for i = 1:numel(x)
                ...
                ReplayTraces(n).Lever(i)     = { MyData(start_idx:stop_idx, LeverCol) };
                ReplayTraces(n).Motor(i)     = { MyData(start_idx:stop_idx, MotorCol) };
                ReplayTraces(n).Sniffs(i)    = { MyData(start_idx:stop_idx, RespCol) };
                ReplayTraces(n).Licks(i)     = { MyData(start_idx:stop_idx, LickCol) };
                ReplayTraces(n).Trial(i)     = { MyData(start_idx:stop_idx, TrialCol) };
                ReplayTraces(n).Rewards(i)   = { MyData(start_idx:stop_idx, RewardCol) };
                
                % extract the timestamps incase it can't be reconstructed from indices
                ReplayTraces(n).Timestamps(i)  = { MyData(start_idx:stop_idx, 1) };
            end
        else
            %ReplayTraces = [];
        end
    ...
    % concatenate stuff across tuning sessions
    MyTrials_m = vertcat(MyTrials_m, [MyTrials n*ones(size(MyTrials,1),1)]);
    TrialSequence_m = vertcat(TrialSequence_m, [TrialSequence n*ones(size(TrialSequence,1),1)]);
    MyParams_m = vertcat(MyParams_m, [MyParams n*ones(size(MyParams,1),1)]);
    
    % keep track of time when file was created
    X = dir(FileName(n,:));
    TuningFileTS(n,:)    = datevec(X.date);
