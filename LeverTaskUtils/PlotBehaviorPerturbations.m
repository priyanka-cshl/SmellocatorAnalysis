function [] = PlotBehaviorPerturbations(timestamps,Lever,Sniffs,Licks,Rewards,Trial,TZ,Perturbation,OdorBoxHeight)
if nargin<8
    OdorBoxHeight = 5;
end
...
% for plotting perturbation periods
handles.perturb = fill(NaN,NaN,[1 1 1]);
handles.perturb.EdgeColor = 'none';
colormap(brewermap([100],'rdbu'));
set(gca,'CLim',120*[-1 1]);
...
if ~isempty(Perturbation)
    [handles] = PlotToPatch_Perturbation(handles, Perturbation, timestamps, [0 OdorBoxHeight]);
end
