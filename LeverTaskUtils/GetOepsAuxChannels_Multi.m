function [TTLs,ReplayTTLs,BehaviorTrialIdx, EphysTuningTrials,AuxData] = GetOepsAuxChannels_Multi(myKsDir, BehaviorTrials_multi, TuningTrials, TrialSequence, varargin)
...
%% Align ephys and behavior (closed loop trials)
sess = 1; TrialIdx = [];
while sess <= size(BehaviorTrials_multi,2)
    BehaviorTrials = BehaviorTrials_multi{sess}.TimeStamps;
    
    if sess == 1
        % check initial alignment of ephys and behavior - in case acq
        % started later etc
        ...
        % check if trial sequence for the first session matches
        y = corrcoef(BehaviorTrials(:,3),TTLs.Trial(1:size(BehaviorTrials,1),3));
        if y(2,1)<0.99
            TTLs = [];
            EphysTuningTrials = [];
            AuxData = [];
...
if ~isempty(TuningTrials)
    [EphysTuningTrials] = AlignPassiveTuningTrials_Multi(TuningTrials, TTLs, TrialIdx, TrialSequence);
else
    EphysTuningTrials = [];
end
