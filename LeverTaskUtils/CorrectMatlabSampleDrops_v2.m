...
function [Trial] = ...
    CorrectMatlabSampleDrops_v2(MyData, MySettings, DataTags)
...
LeverThresh = median(MySettings(:,11));% - 0.1; % 4.8 can be too strict for batch Q
trialflag = [];
        ...
        if ~isempty(Initiations)
            ...
            if isempty(OdorStart)
                if size(Initiations,1) > 1
                    % sometimes due to noise around the threshold last
                    % initiation gets splits into two
                    if (Initiations(end,1)-Initiations(end-1,2) < 5) && (Initiations(end,2)-Initiations(end-1,1) > TriggerHold)
                        Initiations(end-1,2) = Initiations(end,2);
                        Initiations(end,:) = [];
                        OdorStart = find(diff(Initiations,1,2)>=(TriggerHold-1),1,'last');
                        disp('found split initiations');
                    else
