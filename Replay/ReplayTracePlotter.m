...
function [MyTraces,timestamps,PSTH,Raster] = ReplayTracePlotter(Replay, TrialInfo, SingleUnits, TTLs, PassiveReplayTraces, varargin)
...
for x = 1:numel(allreplays) % for every unique replay stretch
    ...
    % plot passive traces
    for p = 1:size(PassiveReplayTraces.Sniffs,2)
