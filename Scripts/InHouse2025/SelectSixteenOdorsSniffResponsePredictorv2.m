%function [] = SixteenOdorsSniffResponsePredictor() %(myDir,myStimFile)
...
%% load the kernels and FRs to get predicted spikes
switch whichmouse
    case 'T3'
...
for n = 1:nUnits
    ...
    %thisUnitSpikes = SingleUnits(n).predictedspikes;
...
%     % predicted spikes
%     for j = 3:size(Sniffs2Use{1},1) % every sniff
%         x1 = abs(floor(Sniffs2Use{1}(j,end-1)/5)) - 10;
%         x2 = x1 + 99;
%         x1 = x1 - 9;
%         thisSniffPSTH = FRs(x1:x2,2,whichUnit); % 2 for using the poisson GLM fits
%         thisSniffSpikes = PechePourPoisson(multiplier*thisSniffPSTH,0.1)/10 - 0.1;
%         PredictedSpikesPlot = vertcat(PredictedSpikesPlot, [thisSniffSpikes 0+(j*ones(numel(thisSniffSpikes),1))]);
%     end
...
%     sniffsDone = sniffsDone + maxsniffs;
    ...
    for q = 1:size(sniffgrouping,1)
        ...
        if q == 3
            [SpikeRaster, maxsniffs, ~] = GetSniffLockedSpikesAndPSTH(chunkedsniffs, thisUnitSpikes, 'window', window, 'binsize', binsize);
            plot(SpikeRaster{1}(:,1), SpikeRaster{1}(:,2), '.k','Markersize', 0.5);
            sniffsDone = sniffsDone + maxsniffs + 5;

            % predicted spikes
            for j = 1:size(chunkedsniffs{1},1) % every sniff
                x1 = abs(floor(chunkedsniffs{1}(j,end-1)/5)) - 10;
                x2 = x1 + 99;
                x1 = x1 - 9;
                thisSniffPSTH = FRs(x1:x2,2,whichUnit); % 2 for using the poisson GLM fits
                thisSniffSpikes = PechePourPoisson(multiplier*thisSniffPSTH,0.1)/10 - 0.1;
    ...
    for odor = 1:numel(StimSettings.miniOdors)
        ...
        % predicted spikes
        for j = 1:size(Sniffs2Use{1},1) % every sniff
            x1 = abs(floor(Sniffs2Use{1}(j,end-1)/5)) - 10;
            x2 = x1 + 99;
            x1 = x1 - 9;
            thisSniffPSTH = FRs(x1:x2,2,whichUnit); % 2 for using the poisson GLM fits
            thisSniffSpikes = PechePourPoisson(multiplier*thisSniffPSTH,0.1)/10 - 0.1;
            PredictedSpikesPlot = vertcat(PredictedSpikesPlot, [thisSniffSpikes sniffsDone+(j*ones(numel(thisSniffSpikes),1))]);
        end
    ...
    plot(PredictedSpikesPlot(:,1), PredictedSpikesPlot(:,2), '.k','Markersize', 0.5);
    set(gca,'TickDir','out','YTick',[],'YLim',b, 'XLim',[-0.1 1]);

    if rem(n,unitsPerFig)==0 || n == nUnits
        subplot(nRows,nCols,whichsubplot+2);
    ...
    if (rem(n,unitsPerFig)==0 || n == nUnits)
        if savefigs
            ...
            if n == unitsPerFig
                exportgraphics(gcf, ...
                    fullfile(myKsDir,'OdorMaps','SniffOdorSummaryFirstPulse2.pdf'),...
                    'ContentType','vector');
            else
                exportgraphics(gcf, ...
                    fullfile(myKsDir,'OdorMaps','SniffOdorSummaryFirstPulse2.pdf'),...
...
for n = 1:numel(myUnits)
    ...
    whichUnit = myUnits(n);
    Kernels = kernels.pGLM(:,:,whichUnit);
    % baseline kernel
    plot(Kernels(:,1),'Color','k');

    % plot the 10 mini odors
    for odor = 1:numel(StimSettings.miniOdors)
        whichstim = find(StimulusList == StimSettings.miniOdors(odor));
        plot(Kernels(:,whichstim),'Color',mycolors(whichstim+1,:));
    end
