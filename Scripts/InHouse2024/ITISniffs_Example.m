...
if sortbyprevious
    ...
    ITISniffs = sortrows(ITISniffs,9,'descend'); % previous sniff durations are negative
end
...
for x = 1:size(ITISniffs,1)    
    if plotevents
        ...
        InhalationTimes = [ITISniffs(x,[9 10]) 0 ITISniffs(x,3)];
        ...
        if warptype % warp by inhalation duration
            InhalationTimes = InhalationTimes * (mean(ITISniffs(:,3))/ITISniffs(x,3));
            ExhalationTimes = ExhalationTimes * (mean(ITISniffs(:,3))/ITISniffs(x,3));
        end
        if ~shade_exhalation
    ...
    if plotspikes || psth
        ts = [ITISniffs(x,9) 0 ITISniffs(x,4) ITISniffs(x,3)] + ITISniffs(x,11); 
        ...
        if warptype
            thisSniffSpikes = thisSniffSpikes * (mean(ITISniffs(:,3))/ITISniffs(x,3));
            tmax = tmax * (mean(ITISniffs(:,3))/ITISniffs(x,3));
        end
...
nSniffs(examples) = size(ITISniffs,1);
...
if ~sortbyprevious
    print('/Users/Priyanka/Desktop/LABWORK_II/Presentations/InHouse2024/ITIsniffsExampleRasters.eps',...
    '-depsc','-tiff','-r300','-painters');
else
    print('/Users/Priyanka/Desktop/LABWORK_II/Presentations/InHouse2024/ITIsniffsExampleRasters_PreviousSniff.eps',...
    '-depsc','-tiff','-r300','-painters');
end
...
set(gcf,'Renderer','painters');
if ~sortbyprevious
    print('/Users/Priyanka/Desktop/LABWORK_II/Presentations/InHouse2024/ITIsniffsExampleRasters_FRs.eps',...
        '-depsc','-tiff','-r300','-painters');
else
    print('/Users/Priyanka/Desktop/LABWORK_II/Presentations/InHouse2024/ITIsniffsExampleRasters_PreviousSniff_FRs.eps',...
        '-depsc','-tiff','-r300','-painters');
end
