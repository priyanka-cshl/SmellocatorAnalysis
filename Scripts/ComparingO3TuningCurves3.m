...
for unit = 1:N
    for odor = 1:3
        ...
        Z = vertcat(Z, [Y' (1:4)' unit*ones(4,1) odor*ones(4,1) C']);
    end
end

%% plotting cumulative distributions - residuals
figure;
% Z
...
figure;
% Z
% dimension 5 = correlations
% dimension 2 = paired condition for which the residual was calculated
% dimension 3 = unit identity
% dimension 4 = odor identity
XVar = (-1:0.02:1)'; %logspace(-1,2)'; %
MyColors = [Plot_Colors('r'); Plot_Colors('t'); Plot_Colors('k')];
set(groot,'defaultAxesColorOrder',MyColors);
for odor = 1:4
    % binsize 10
    ...
    if odor < 4
        AllResiduals = [];
        for conditions = 1:4
            AllResiduals(:,conditions) = Z(intersect(find(Z(:,2)==conditions),find(Z(:,4)==odor)),5);
        end
        [ResidualDist] = CumDist(AllResiduals,XVar);
    else
        AllResiduals = [];
        for conditions = 1:4
            AllResiduals(:,conditions) = Z(find(Z(:,2)==conditions),5);
        end
        [ResidualDist] = CumDist(AllResiduals,XVar);
    end
    area(XVar,ResidualDist(:,4),'LineStyle','none','FaceColor',[0.8 0.8 0.8]);
...
%% Residuals vs. Correlations
figure;
for odor = 1:4
    % binsize 10
    if odor < 4
        AllResiduals = []; AllCorrs = [];
        for conditions = 1:4
            AllResiduals(:,conditions) = Z(intersect(find(Z(:,2)==conditions),find(Z(:,4)==odor)),1);
            AllCorrs(:,conditions) = Z(intersect(find(Z(:,2)==conditions),find(Z(:,4)==odor)),5);
        end
    else
        AllResiduals = []; AllCorrs = [];
        for conditions = 1:4
            AllResiduals(:,conditions) = Z(find(Z(:,2)==conditions),1);
            AllCorrs(:,conditions) = Z(find(Z(:,2)==conditions),5);
            subplot(2,4,conditions); hold on
            axis square
            scatter(AllCorrs(:,conditions),AllResiduals(:,conditions),'ok');
            subplot(2,4,conditions+4); hold on
            axis square
            scatter(AllCorrs(:,conditions),AllResiduals(:,conditions),'ok');
        end
...
figure;
hold on
scatter(AllCorrs(:,4),AllResiduals(:,4),20,'k','filled');
scatter(AllCorrs(:,1),AllResiduals(:,1),20,Plot_Colors('r'),'filled');
scatter(AllCorrs(:,2),AllResiduals(:,2),20,Plot_Colors('t'),'filled');
set(gca,'YLim',[0 0.5])
%% get some numbers
...
for n = 1:numel(ModUnits) %size(Curve_CL{1},4)
    ...
    for odor = 1:3
        ...
        r = Z(intersect(find(Z(:,3)==n),find(Z(:,4)==odor)),[1 5]);
        title(mat2str([i r(1:3,1)' r(1:3,2)'],2));
    end
end
