...
%% Assembling half-tuning curves for closed loop trials
for whichodor = 1:3
    % 1. Pick the right trials - no perturbation, and a given odor
    allTrials = intersect(find(cellfun(@isempty, TrialInfo.Perturbation)), ...
        find(TrialInfo.Odor==whichodor));
    
    allTrials = allTrials(randperm(numel(allTrials)));
    
    whichTrials = allTrials(1:floor(numel(allTrials)/2));
    XVar = []; YVar = [];
    for whichUnit = 1:numel(MyUnits) % for every Unit
        counts = [1 0];
        thisUnitSpikes = AlignedSpikes(:,MyUnits(whichUnit));
...
%     end
    ...
    [Curve_CL1{whichodor}, XBins] = SmellocatorTuning('Odor',125-XVar,YVar);
    
    whichTrials = allTrials(floor(numel(allTrials)/2):end);
    XVar = []; YVar = [];
    for whichUnit = 1:numel(MyUnits) % for every Unit
        counts = [1 0];
        thisUnitSpikes = AlignedSpikes(:,MyUnits(whichUnit));
...
%     end
    % add one column for randomized locations
    XVar(:,2) = XVar(randperm(size(XVar,1)),1);
    [Curve_CL2{whichodor}, XBins] = SmellocatorTuning('Odor',125-XVar,YVar);
end
