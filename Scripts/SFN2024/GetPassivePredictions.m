function [TS_down,PSTHs,PredictedPSTHs,WulfPSTHs] = GetPassivePredictions(SessionName)
% outpputs:
...
TS_temp     = PassiveOut.Timestamps{1};
...
if isfield(PassiveOut,'SniffsDigitized')
    AllSniffs       = interp1(TS_temp,PassiveOut.SniffsDigitized{1},TS_down,'nearest');
    OdorState       = interp1(TS_temp,PassiveOut.Odor{1},TS_down,'nearest');
    ManifoldState   = interp1(TS_temp,PassiveOut.Manifold{1},TS_down,'nearest');
    ...
    OdorLocations = PassiveOut.SniffsLocationed{1};
...
%     TrialState  = interp1(TS_temp,PassiveOut.Trial{1},TS_down,'nearest');
...
WulfPSTHs = LoadWulfGLMOutputPassive(wulfPath,nUnits,TS_down);
