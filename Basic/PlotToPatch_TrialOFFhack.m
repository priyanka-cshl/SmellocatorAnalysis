function [handle_in] = PlotToPatch_TrialOFFhack(handle_in, trial_in, timestamp_in, offsets, whichcase)
if nargin<5
    whichcase = 0;
end
...
%for i = 1:3 % three odors
    ...
    if any(on_indices)
        ...
        if (ts_on_Y(end) == 1 || whichcase)
            ...
            handle_in.trial_off.Vertices = [ts_on_X ts_on_Y];
            handle_in.trial_off.Faces = 1:length(ts_on_X);
        end
    end
%end
end