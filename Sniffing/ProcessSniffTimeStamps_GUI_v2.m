function varargout = ProcessSniffTimeStamps_GUI_v2(varargin)
% PROCESSSNIFFTIMESTAMPS_GUI_V2 MATLAB code for ProcessSniffTimeStamps_GUI_v2.fig
%      PROCESSSNIFFTIMESTAMPS_GUI_V2, by itself, creates a new PROCESSSNIFFTIMESTAMPS_GUI_V2 or raises the existing
%      singleton*.
%
%      H = PROCESSSNIFFTIMESTAMPS_GUI_V2 returns the handle to a new PROCESSSNIFFTIMESTAMPS_GUI_V2 or the handle to
%      the existing singleton*.
%
%      PROCESSSNIFFTIMESTAMPS_GUI_V2('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in PROCESSSNIFFTIMESTAMPS_GUI_V2.M with the given input arguments.
%
%      PROCESSSNIFFTIMESTAMPS_GUI_V2('Property','Value',...) creates a new PROCESSSNIFFTIMESTAMPS_GUI_V2 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before ProcessSniffTimeStamps_GUI_v2_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to ProcessSniffTimeStamps_GUI_v2_OpeningFcn via varargin.
%
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @ProcessSniffTimeStamps_GUI_v2_OpeningFcn, ...
                   'gui_OutputFcn',  @ProcessSniffTimeStamps_GUI_v2_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before ProcessSniffTimeStamps_GUI_v2 is made visible.
function ProcessSniffTimeStamps_GUI_v2_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to ProcessSniffTimeStamps_GUI_v2 (see VARARGIN)

% Choose default command line output for ProcessSniffTimeStamps_GUI_v2
...
% UIWAIT makes ProcessSniffTimeStamps_GUI_v2 wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = ProcessSniffTimeStamps_GUI_v2_OutputFcn(hObject, eventdata, handles) 
...
if isfield(handles.SniffTrace,'MFS')
    axes(handles.MFS);
    set(handles.mfsTrace,'XData', handles.SniffTrace.Timestamps, 'YData', handles.SniffTrace.MFS);
    zoom off
    set(handles.peaksmfs,'XData',handles.SniffTrace.Timestamps(handles.SniffsTS(:,8)),...
        'YData',handles.SniffTrace.MFS(handles.SniffsTS(:,8)));
    set(handles.valleysmfs,'XData',handles.SniffTrace.Timestamps(handles.SniffsTS(:,9)),...
        'YData',handles.SniffTrace.MFS(handles.SniffsTS(:,9)));
    set(gca,'XLim',handles.SniffTrace.Timestamps(1)+[0 str2double(handles.WindowSize.String)], ...
    'YTick', 2.5,  'XTick', []);
end
