...
[SniffTS_TH, inflexion_points_TH, THout] = ProcessThermistorData([TS TH]); %,'plotting',true);
THfilt = THout(:,3);

% using the pressure sensor
[SniffTS_PS, inflexion_points_PS, PSout] = ProcessThermistorData([TS PS]); %,'plotting',true);
PSfilt = PSout(:,3);

% getting pressure from thermistor
PS2 = diff(TH);
PS2(end+1) = PS2(end);
[SniffTS_PS2, inflexion_points_PS2, PSout2] = ProcessThermistorData([TS PS2]); %,'plotting',true);
PSfilt2 = PSout2(:,3);
