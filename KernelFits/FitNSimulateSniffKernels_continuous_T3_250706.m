...
%%
savefigs = 1;
nRows = 13; nCols = 6; nhalfrows = 6;

InputVector = []; %zeros(6,nBins); % sniff, air, odor1, odor2, odor3, motor location
InputVector(1,:) = SniffTraceDS(:,2);
InputVector(2:7,:) = 0;

for thisunit = 1:nUnits
    [baseline,kernels,locationcoef] = ParseSniffKernels(fittedkernel{thisunit},'independentcoeffs', true);
    % get psth
    PSTHout = SniffKernels2continuousPSTH(baseline,kernels,locationcoef,InputVector);
    PSTHout(PSTHout<0) = 0;
    %PSTHout(:,1:length(KernelDS)) = [];
    thisUnitSpikes = PechePourPoisson(PSTHbinsize*PSTHout,PSTHbinsize/1000);
    [SpikeRaster, maxsniffs] = GetSniffLockedSpikes(Sniffs2Use, thisUnitSpikes);
    
    if ~rem(thisunit-1,nCols)
        FigureName = ['Units ',num2str(thisunit),' - ',num2str(k+nCols-1)]; % one figure per cell
        figure('Name',FigureName);
    end
    
    whichcol = rem(thisunit-1,nCols) + 1;
    subplot(nRows,nCols,whichcol);
    plot(kernels(:,1));
    
    whichplots = (7:nCols:((nhalfrows+1)*nCols)) + (whichcol-1);
    subplot(nRows,nCols,whichplots);
    plot(SpikeRaster{1}(:,1), SpikeRaster{1}(:,2), '.k','Markersize', 0.5);
    
    
    % actual spikes
    thisUnitSpikes = SingleUnits(thisunit).spikes;
    [SpikeRaster, maxsniffs] = GetSniffLockedSpikes(Sniffs2Use, thisUnitSpikes);
    
    whichplots = (nhalfrows)*nCols + whichplots;
    subplot(nRows,nCols,whichplots);
    plot(SpikeRaster{1}(:,1), SpikeRaster{1}(:,2), '.k','Markersize', 0.5);
    
    if ~rem(thisunit,nCols)
        set(gcf,'Position',[1985        -364         933        1154]);
        if savefigs
            set(gcf,'Color','w');
            set(gcf,'renderer','Painters');
            print(['/mnt/data/simulation/predictedvsobservedsnifftuning',num2str(thisunit),'.eps'],'-depsc','-tiff','-r300','-painters');
            close(gcf);
        end
    end
