...
MatFile = fullfile(SavePath,[SessionName,'_',FitStyle,'_20ms_new.mat']);
...
if isfield(TracesOut,'SniffsDigitized')
    AllSniffs   = interp1(TS_temp,TracesOut.SniffsDigitized{1},TS_down);
    OdorState   = interp1(TS_temp,TracesOut.Odor{1},TS_down);
    % parse sniffs into ITI, air, odor1, odor2, odor3
    %InputVector(1,:) = AllSniffs; 
    
    InputVector(1,:) = AllSniffs.*(OdorState==0); % only ITI sniffs
    InputVector(2,:) = AllSniffs.*(OdorState>0); % air sniffs
    ...
    TrialState  = interp1(TS_temp,TracesOut.Trial{1},TS_down);
