...
params.addParameter('onlyparams', false, @(x) islogical(x) || x==0 || x==1);
...
onlyparams      = params.Results.onlyparams;
...
for x = 1:size(AllSniffs,1)
    ...
    SniffParams(x,1:10) = [ AllSniffs(x,[2 13]) ts(4:5) - ts(3) ...
        AllSniffs(x,[1 3]) ...
    ...
    if onlyparams
        SniffParams(x,11) = ts(3); % actual sniff start time
    end
end
end